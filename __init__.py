...
# License: GPLv3 Copyright: 2018, acgnd.com <lu@acgnd.com>
...
from calibre import as_unicode, browser, random_user_agent, prints
...
class Worker(Thread):  # Get details {{{
    ...
    def parse_details(self, raw, root):
		...
		try:
			price = self.parse_price(root)
			idtype = 'price'
			if not price:
			    #return 0 if find none
			    price = '0'
			mi.set(idtype, price)
            mi.set_identifier(idtype, price)
			price = self.parse_price2(root)
			idtype = 'price2'
			if not price:
                #return 1 if find none
				price = '1'
			mi.set(idtype, price)
            mi.set_identifier(idtype, price)
		except:
        ...
        if non_hero:
            ...
            if pd:
                ...
                try:
					...
					mi.publisher = mi.publisher.replace('・','·')
                except:
                    self.log.exception(
    ...
    def parse_price2(self, root):
		ans = ''
		price = ''
        p2 = root.xpath('//div[@id="priceBlock"]/table/tbody/tr//td[@class="listPrice"]')
		pf = root.xpath('//tbody[@id="paperback_meta_binding_winner"]/tr//td[@class=" price "]')
		pg = root.xpath('//tbody[@id="paperback_meta_binding_winner"]/tr//td[@class="tmm_olpLinks "]')
		po = root.xpath('//tbody[@id="paperback_meta_binding_winner"]/tr//td[@class="tmm_olpLinks "]/a')
		p5 = root.xpath('//span[@class="a-size-base a-color-secondary"]')
		if p2:
            price = p2[0].text.strip()
			prints('\nParse price p2: \'%s\''%price)
        elif pf:
		    price = pf[0].text.strip()
			if price == '--':
				price = ''
            else:
				prints('\nParse price paperback: \'%s\''%price)
        if price == '':
            if pg:
                price = pg[0].text.strip()
				if price == '--':
					price = ''
				else:
					prints('\nParse price paperback new : \'%s\''%price)
        if price == '':
            if po:
                price = po[0].text.strip()
				if price == '--':
					price = ''
				else:
					prints('\nParse price paperback old : \'%s\''%price)
		if price == '':
            if p5:
				price = p5[0].text.strip()
		if price:
		    ans = price.rpartition(' ')[-1].strip()
		    ans = ans.replace('|', '')
		    ans = ans.replace(',', '')
		else:
            prints('\nParse price of paper failed: ')	
		return ans

    def parse_price(self, root):
		ans = ''
		price = ''
        p1 = root.xpath('//span[@id="declarative_1"]/table/tbody/tr//td[@class="a-text-right.dp-price-col"]/div/div/div[@class="a-fixed-right-grid-col.a-col-left"]/span')
		p2 = root.xpath('//span[@class="a-size-base a-color-price a-color-price"]')
		p3 = root.xpath('//div[@id="buybox"]/div/table/tbody//tr[@class="kindle-price"]//td[@class="a-color-price.a-size-medium.a-align-bottom"]')
		#buybox > div > table > tbody > tr.kindle-price > td.a-color-price.a-size-medium.a-align-bottom
		if p1:
            price = self.tostring(p1[0], encoding=unicode,method='text').strip()
		elif p2:
			price = p2[0].text.strip()	
		if price == '￥ 0':#Kindle Unlimit
			if p3:
				price = p3[0].text.strip()
		if price:
			prints('\nParse price: \'%s\''%price)
			ans = price.rpartition(' ')[-1].strip()
			prints('\nParse price: \'%s\''%ans)
...
class AmazonPrice(Source):

    name = 'AmazonPrice'
    version = (3, 14, 0)
    minimum_calibre_version = (2, 82, 0)
    description = _('Downloads Price, metadata and covers from Amazon')
...
if __name__ == '__main__':  # tests {{{
    ...
    from calibre.ebooks.metadata.sources.test import (test_identify_plugin,
            isbn_test, title_test, authors_test, tags_test, series_test, comments_test, pubdate_test)							  
    com_tests = [  # {{{
    ...
    cn_tests = [  # {{{
        (
            {'identifiers': {'amazon_cn': 'B00VJHO6JO'}},
    ...
    def do_test(domain, start=0, stop=None, server='auto'):
        ...
        test_identify_plugin(AmazonPrice.name, tests, modify_plugin=lambda p: (
    ...
    #do_test('jp')
    #do_test('cn')
    #do_test('com',0,1,'amazon')
	do_test('jp',0,3,'amazon')
# }}}
